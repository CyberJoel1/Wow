...
const SectionPublication = (props: Props) => {
  ...
  const getAllPublication = async () => {
    const response = await QueryPublication.findAllPublicationForClient(profile);
  ...
  return (
    <div>
      <div className='mx-[7%] border border-indigo-200 rounded-lg mt-6 mb-3 drop-shadow-lg'>
        ...
        {!equalUser && <SchemaInsertPublication setRenderData={setRenderData} renderData={renderData} />}
